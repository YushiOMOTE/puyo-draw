...
- Pressing anywhere in the field begins direct manipulation and moves the upper preview to the pressed column. Horizontal pointer movement then moves the visible pair left or right from that column, snapping one column at a time with hysteresis. If the pair cannot occupy the pressed column at spawn height, it retains its current spawn column. The pair remains at the upper spawn height throughout the preview.
