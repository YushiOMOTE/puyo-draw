...
- A Ranking section immediately below the Your move and Ama's choice comparison lists up to five Ama-ranked legal Current placements. Its table shows rank, a text description of the candidate move, and the candidate's highest score among the six tested futures. Ranking still follows the six-future aggregate, so the displayed maximum is evidence rather than the sorting value. Each row has a Preview action that shows the read-only pre-resolution placement below the table. Only one candidate preview is visible at a time; selecting another replaces it, and a close button hides it.
