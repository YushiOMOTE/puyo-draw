...
- The review identifies whether the user's move is Ama's first choice, a score-tied alternative, or a different recommendation. It reports the user's ordinal rank among the returned legal moves, both six-branch average maximum-chain scores, their difference, and how many branches favor the user, tie, or favor Ama's aggregate-best move.
- The reported values are sampled search results, not an absolute grade or proof that a move is good or bad. The first version does not attribute Ama's choice to individual static evaluation features.
