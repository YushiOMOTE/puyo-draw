...
- Pressing a field cell selects its column and opens a flick menu. Its center previews and drops the pair straight; down previews and drops it upside down; right and left rotate it 90 degrees; up shows a cancel option. Moving onto a placement option updates the upper pair preview to that placement orientation, while moving up restores the spawn preview. Releasing in the center drops straight, and releasing up cancels.
- The selected column is used for all four actions. Quarter-turn placements use the existing Tsu-style wall kicks; the 180-degree action intentionally skips wall kicks. Only the pointer that started the gesture may update or finish it.
