...
- The current main-chain ignition cell is shown as a colored double dashed circle with a lightning mark. It pulses when the current pair can fire it. A current-pair marker keeps its centered sparkle and adds the lightning as a corner badge when meanings share a cell.
