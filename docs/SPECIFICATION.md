...
- The Ama-style construction evaluation rewards connected pairs and triples, accessible groups, useful different-color contacts, resource efficiency, low and reachable ignition, and multiple equal-length ignition routes. It penalizes buried isolated puyos, uneven surfaces, excessive left/right imbalance, dangerous height, hidden-row occupancy, and split horizontal landings across large height differences.
- A non-clearing construction route may not raise any column above eleven occupied puyos. This reserves the top visible row and hidden row for safety; a clearing route that reduces the field may still be considered as an emergency fallback.
- Existing main-chain size ranks ahead of structural evaluation. Unknown-pair coverage then ranks ahead of horizon main-chain size and resource efficiency. Distinct first-pair placements are retained as alternatives, and repeated height profiles are capped so one field shape cannot consume the beam.
- Any non-clearing continuation is preferred over an automatic clear in the long-chain planner. A clearing route appears only as an explicitly labeled emergency-clear fallback when no stable continuation survives. The separate emergency-attack Suggestion remains responsible for ranking intentional attacks.
...
- The existing connected puyos that form the first clearing group of the current main chain are shown with bright color-matched highlight rings. The empty cell where an additional ignition puyo would land is not marked. The target rings pulse when the current pair can fire the chain. When a current, Next, or Next Next recommendation occupies a highlighted cell after an intervening clear, the recommendation marker takes priority.
- The planner does not display a projected horizon ignition or post-preview roadmap cells; those conditional markers are intentionally omitted to keep the current action legible.
- The toast reports the horizon main-chain size, connected-resource efficiency, the current ignition-target color and group size, and evaluated unknown-pair coverage. When no one-puyo main chain exists yet, it identifies the move as main-chain base construction.
