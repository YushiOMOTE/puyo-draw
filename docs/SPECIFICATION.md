...
- The roadmap shows a bounded set of faint colored dashed circles. Each is a currently accessible single-puyo cell with the best local structural gain in its column after the visible route; it is guidance rather than a promise about unseen future pairs.
- The current main-chain ignition cell is shown as a colored double dashed circle with a lightning mark. It pulses when the current pair can fire it. The projected horizon ignition uses a lighter lightning marker. A current-pair marker keeps its centered sparkle and adds the lightning as a corner badge when meanings share a cell.
- The toast reports the current or projected main-chain size, connected-resource efficiency, current ignition, evaluated unknown-pair coverage, and projected ignition. When no one-puyo main chain exists yet, it identifies the move as main-chain base construction.
