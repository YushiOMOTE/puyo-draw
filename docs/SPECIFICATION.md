...
- The current main-chain ignition cell is shown as a colored double dashed circle with a lightning mark and pulses when the current pair can fire it. When a current, Next, or Next Next recommendation occupies the same cell, the recommendation marker takes priority and replaces the ignition marker; ignition styling is never recolored to the recommended puyo.
...
- The toast reports the horizon main-chain size, connected-resource efficiency, current ignition, and evaluated unknown-pair coverage. When no one-puyo main chain exists yet, it identifies the move as main-chain base construction.
