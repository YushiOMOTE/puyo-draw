...
- Tokopuyo legal placements match Ama's move generator, including spawn-path checks, floor kicks, quick turns, the special fourteenth row, and the standard third-column death check. Manual movement and rotation can reach every placement accepted by that generator, and a drop cannot commit a placement it rejects. The active-pair preview may move into a position rejected as a final placement; attempting to drop there shows a brief localized reason, including a blocked spawn, excessive target height, occupied special-fourteenth-row cell, blocked path, or unavailable wall-kick foothold.
- Tokopuyo garbage mode replaces the active Current with one movable garbage puyo. The garbage puyo can move horizontally and drop like a normal active piece; rotation is disabled because a single puyo has no meaningful orientation. Each garbage drop is one Undo/Redo history entry, does not consume or advance the normal Current, and leaves garbage in the field. Garbage puyos never form clearing groups, so garbage drops do not start chains or add score; they can still be cleared later when adjacent to a normal color group that clears. Turning the mode off restores the Current that was active when the mode began. The mode toggle itself is not a history entry.
...
- The chain count and cumulative score describe the most recently committed hand. A non-clearing hand resets both to zero, and both values are shown below the field with the selected language's score and chain labels.
...
- The Tokopuyo right-side rail ends above the bottom control bar and contains, below the previews: Undo, Redo, long-chain Suggestion, emergency-attack Suggestion, and Palette/Garbage controls are hidden. Reset, the Drawing mode switch, and the Tokopuyo garbage mode toggle are on the left rail, with Help pinned at the bottom.
...
- A shared replay with no stored tsumo sequence has no normal Current pair or Next previews in Preview mode. Returning to Tokopuyo starts a fresh random queue only when no stored pair remains; this continuation is outside the shared replay until copied into a new URL.
...
- Copy History URL is available in Preview mode. It serializes the complete configured hand queue, the initial position when the session came from a custom board, the special-row occupancy mask, and the entire retained operation timeline in order, including Redo-side history. The URL does not preserve the current cursor; it always opens at the beginning.
...
- A valid URL opens directly in Preview mode at the beginning of the encoded history, with Current positioned at the first recorded operation. The encoded queue remains available when returning to Tokopuyo, and new play continues from the history position selected in Preview using normal Undo/Redo behavior.
...
The application supports English and Japanese in both Drawing and Tokopuyo modes. The initial language follows the browser locale (Japanese for `ja` locales; English otherwise), and the Help dialog provides a language selector. User-facing copy is maintained separately from application logic in the localization catalog. Toast notifications use the selected language. Toasts are used for reset, undo, redo, clear, and no-chain feedback. Chain progress/completion and suggestion searches, results, and precondition feedback are shown without toasts. Cell placement messages are not shown as toasts.
...
- Ama-ranked Current alternatives whose aggregate six-branch score is 90% or less of the top score are discarded before display; among the survivors, up to four ranked alternatives are cached. The top-scoring alternative is always retained. Pressing Suggestion again cycles through the displayed alternatives without rerunning search. The complete ranked result remains available to Last-Move Review. Committing a pair, Undo, Redo, Reset, or changing mode removes the cache.
...
- The review identifies whether the user's move is Ama's first choice, a score-tied alternative, or a different recommendation. Ranking starts with the user's ordinal rank and six-future average score among the returned legal moves. Future potential and stability report both six-branch average, minimum, and maximum found chain scores.
...
- A Ranking section immediately below the Your move and Ama's choice comparison starts with a full-width user-rank summary and lists up to five Ama-ranked legal Current placements. Directly below the user-rank and user-average-score cards, a 96px-high score bar chart spans every legal placement from rank 1 at left through the lowest rank at right. It shows the highest displayed average, midpoint, and zero values without a redundant vertical-axis title. The user's rank is blue, marked `YOU` above its bar, and every other bar is purple; the chart title has enough separation from the plot for this marker at rank 1. Its table shows rank, a compact physical-color icon for the axis and child puyos with the axis column and child direction, and the candidate's six-future average score. The same iconized placement label appears above a candidate preview. Ranking and all displayed score values use the same six-future average, rounded to the nearest integer for display. Each row has a Preview action that shows the read-only placement below the table. Only one candidate preview is visible at a time; selecting another replaces it, and a close button hides it. Future potential and stability follows Ranking and precedes the Ama evaluation section.
- Potential, stability, immediate priority, and the largest weighted feature gaps use paired or diverging bars. Section explanations and the largest weighted feature gaps have adjacent question-mark disclosures; the Potential, Stability, and Immediate priority metric cards do not duplicate those explanations. Long definitions and raw calculations stay behind the remaining disclosures, and tapping elsewhere in the report closes any open disclosure. The six detailed future rows and complete feature table are collapsed by default.
- Every coaching-report question-mark disclosure keeps its balloon within the visible report card. The shared positioning rule clamps it to the card's horizontal and vertical bounds, places it above the question mark when there is not enough room below, and scrolls the balloon body internally if the available height is smaller than its content.
