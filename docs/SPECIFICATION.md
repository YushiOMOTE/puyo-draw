...
- The review identifies whether the user's move is Ama's first choice, a score-tied alternative, or a different recommendation. Ranking starts with the user's ordinal rank and maximum single-future score among the returned legal moves. Future potential and stability report both six-branch average, minimum, and maximum found chain scores.
...
- A Ranking section immediately below the Your move and Ama's choice comparison starts with a full-width user-rank summary and lists up to five Ama-ranked legal Current placements. Directly below the user-rank and user-maximum-score cards, a 96px-high maximum-score bar chart spans every legal placement from rank 1 at left through the lowest rank at right. It shows maximum, midpoint, and zero values without a redundant vertical-axis title. The user's rank is blue, marked `YOU` above its bar, and every other bar is purple; the chart title has enough separation from the plot for this marker at rank 1. Its table shows rank, a compact physical-color icon for the axis and child puyos with the axis column and child direction, and the candidate's highest score among the six tested futures. The same iconized placement label appears above a candidate preview. Ranking still follows the six-future aggregate, so the displayed maximum is evidence rather than the sorting value. Each row has a Preview action that shows the read-only placement below the table. Only one candidate preview is visible at a time; selecting another replaces it, and a close button hides it. Future potential and stability follows Ranking and precedes the Ama evaluation section.
