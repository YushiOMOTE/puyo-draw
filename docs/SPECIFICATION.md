...
- Pressing anywhere in the field, including cell gaps, begins direct manipulation and moves the upper preview to the pressed column. The start area extends 0.35 cell beyond every field edge without overlapping the sidebar controls; a press beyond the left or right edge targets the nearest outer column. Horizontal pointer movement then moves the visible pair left or right from that column, snapping one column at a time with hysteresis. If the pair cannot occupy the pressed column at spawn height, it retains its current spawn column. The pair remains at the upper spawn height throughout the preview.
- Horizontal and vertical pointer displacement are evaluated independently throughout the gesture, with no separate translation and rotation modes. Each 0.7-cell horizontal step requests one column of movement. Each 0.7-cell upward step requests one 90-degree clockwise rotation, while each downward step requests one 90-degree counterclockwise rotation. Continued vertical movement passes through 180 and 270 degrees and returns to the initial orientation after four steps. A diagonal drag may move and rotate the preview at the same time, and returning toward the press position reverses the corresponding steps.
- Both axes use return hysteresis so small reversals near a step boundary do not make the preview oscillate. Rotations are evaluated only at the current upper preview position. Tokopuyo placement uses no wall kicks or floor kicks: an orientation that would leave the active-pair area or overlap a locked puyo is skipped, while continued vertical movement may select the next valid 90-degree orientation.
- Releasing commits the last valid preview. Moving into one of four visible corner cancel targets arms cancellation; releasing inside that target cancels and restores the pair to spawn. Moving out of the target disarms cancellation. A press that begins inside a corner target must leave and re-enter it before cancellation can arm. Only the pointer that started the gesture may update or finish it.
...
- The protected ignition cell is always shown as a colored double dashed circle with a lightning mark. It pulses when the selected visible sequence can fire it, and the toast identifies the ignition color, column, and readiness. A current-pair marker keeps its centered sparkle and adds the lightning as a corner badge when both meanings share a cell.
