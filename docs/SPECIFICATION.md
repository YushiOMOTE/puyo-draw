...
- Each chart row identifies its fixed alternating unequal-color pair pattern using the session's physical colors. The patterns are synthetic test continuations after Current and Next, not draws from the session seed.
...
- The two comparison boards initially outline the user's locked Current pair and Ama's chosen Current pair. The diagnostic report exposes each heuristic's raw value and weighted contribution, explains how Ama measures it, identifies the best hypothetical multi-chain trigger probe within three same-color additions, and overlays available cell or column evidence on the two read-only boards only after the corresponding Show on boards action is selected. Trigger additions are hypothetical evaluator probes, not real known future pieces. Pair split and immediate-clear action costs are separate from static board signals.
- Potential, variation, immediate priority, and the largest weighted feature gaps use paired or diverging bars. Long definitions and raw calculations stay behind adjacent question-mark disclosures, while the six detailed future rows and complete feature table are collapsed by default.
